...
public class CurlPower extends JPanel {
    ...
    public void paintComponent(Graphics g) {
        ...
        for (a=j; a<k+1; a++)
            initial.add(Main.element.powers.get(a));
    ...
    public static void pass( ) {
        ...
        JLabel label = new JLabel("POWER OF " + Main.element.name);
        frame.add(label);
        ...
        max = 0;
        for (a=j ; a<k+1; a++) {
            if (Math.abs(Main.element.powers.get(a)) > max)
                max = Math.abs(Main.element.powers.get(a));
        }
        //if (Main.element.powers.get(j) != 0 || Main.element.powers.get(k) != 0)
        //    max = Math.max(Math.abs(Main.element.powers.get(j))
        //            , Math.abs(Main.element.powers.get(k)));
